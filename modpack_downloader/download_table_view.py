...
class A2TaskModel(QAbstractTableModel):
    ...
    def update_data(self):
        if self.task_num != len(self.task_manager.task_list):
            self.task_num = len(self.task_manager.task_list)
            self.layoutChanged.emit()

        # only update visible rows
        top_row = self.table.rowAt(0)
        bottom_row = self.table.rowAt(self.table.geometry().height())
        self.dataChanged.emit(self.index(top_row, 0), self.index(bottom_row, self.columnCount() - 1))
